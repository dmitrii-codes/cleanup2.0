...
namespace Cleanup
{
    public class CleanupController: Controller
    {
        ...
        public CleanupController(CleanupContext context, IHostingEnvironment hostingEnvironment)
        {
            _hostingEnvironment = hostingEnvironment; // initiate the hosting environment to store files
            _context = context;
        }
        ...
        public IActionResult MBoard(int id) {
            ...
            if (activeId != null)
            {
                // retrive the messages by event with id and INCLUDE board messages;
            ...
            if (activeId != null)
            {
                ...
                if (activeUser.Token>0)
                {
                    if (ModelState.IsValid)
                    {
                        ...
                        return RedirectToAction("AddPhoto", new {
                            id = freshCleanup.CleanupId
                        });
                    }
                }
                else
        ...
        public IActionResult CloseCleanup(int id)
        {
            ...
            if (activeId != null)
            {
                ...
                if(possibleCleanup.Count == 1 && activeUser.UserLevel == 9) // Confirm that event exists and that user is admin
                {
                    ...
                    _context.SaveChanges();
                    return RedirectToAction("DeleteCleanup", new {
                        id = possibleCleanup[0].CleanupId
                    });
                }
            }
            return RedirectToAction("Index", "User");
        ...
        public IActionResult ProcessPhoto(int id, IFormFile pic)
        {
            ...
            {
                List<CleanupEvent> possibleCleanup = _context.cleanups.Where(c => c.CleanupId == id).Include(c => c.CleaningUsers).ToList();
                if (possibleCleanup.Count == 1) // Confirm that users went to an existing cleanup event and that they allowed to add photos
                {
                    bool ActiveUserAttending = false;
                    foreach (var user in possibleCleanup[0].CleaningUsers)
                    {
                    ...
                    if (pic != null && (ActiveUserAttending || (int)activeId == possibleCleanup[0].UserId)) // if a picture was uploaded
                    {
                        Random rand = new Random();
                        // stores a string of where the new file root should be
                        var filename = Path.Combine(_hostingEnvironment.WebRootPath + "/images/trash", Path.GetFileName(pic.FileName));
                        // creates an array of the file string before the extension and after, so we can add the random numbers in between
                        String[] newfile = { "", "" };
                        ...
                        Image newImage = new Image{
                            CleanupEventId = id,
                            FileName = splitRootFile[2]
                        };
                        _context.Add(newImage);
                        _context.SaveChanges();
                        // After new photo added, redirect to upload page so user can add more (up to 5 max)
                        return RedirectToAction("AddPhoto", new {
                            id = id
                        });
                    }
        ...
        public IActionResult DeclineCleanupReport(int id)
        {
            ...
            if (activeId != null)
            {
                User activeUser = _context.users.Single(u => u.UserId == (int)activeId);
                List<CleanupEvent> possibleCleanup = _context.cleanups.Where(c => c.CleanupId == id).ToList();
                // Confirm that event exists and that user is admin
                if(possibleCleanup.Count == 1 && activeUser.UserLevel == 9 && possibleCleanup[0].Pending == true)
                {
                    possibleCleanup[0].Pending = false;
                    _context.SaveChanges();
                    return RedirectToAction("AdminPage");
        ...
        public IActionResult Join(int id)
        {
            ...
            if (activeuser != null)
            {
                ...
                if(possibleCleanup.Count == 1 && possibleCleanup[0].CleaningUsers.Count < possibleCleanup[0].MaxCleaners)
                {
                    ...
                    _context.SaveChanges();
                    return RedirectToAction("ViewCleanup", new {
                        id = possibleCleanup[0].CleanupId
                    });
                }
            }
