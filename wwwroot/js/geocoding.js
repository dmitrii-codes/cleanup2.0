$(document).ready(function() {
    ...
    $('#newcleanup').submit(function(event) {
        ...
        if (remainingTokens <= 0) {
            ...
            if (address.length > 0) {
                $.get("https://maps.googleapis.com/maps/api/geocode/json?address=" + address.split(' ').join('+') + "&key=AIzaSyBDTyblNr_Q6boR90WUWLlbhPVlc2u_6h8",
                    function(response) {
                ...
                $.get("https://maps.googleapis.com/maps/api/geocode/json?latlng=" + document.getElementById('Lat').value + "," + document.getElementById('Lng').value + "&key=AIzaSyBDTyblNr_Q6boR90WUWLlbhPVlc2u_6h8",
                    function(response) {
                        if (response["status"]=="OK"){
